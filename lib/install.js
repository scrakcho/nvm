...
async function downloadNode(version, proxyparam, verifyssl) {
  ...
  console.log(ck`<green>Downloading Node ${version}...</>`);
...
async function install(targetPath, version) {
  ...
  console.log(ck`<magenta>Installing Node ${version}...</>`);
  if (!fs.existsSync(nodeCachePath)) {
    return false;
  }
  ...
  try {
    ...
    console.log(ck`<green>Node.js ${version} installed.</>`);
  } catch (err) {
    try {
      await rmdir(path.join(targetPath, nodeFileName));
    } catch {}

    console.log(ck`<red>Node ${version} installed failed</>`, err);
    console.log(ck`<green>Try to clean cache with 'nvm cleanup' and try again.</>`);
  }
}
...
module.exports = async function(version, proxy, verifyssl) {
  ...
  if (versionParts[0] < 4 || (versionParts[0] === 4 && versionParts[1] < 5)) {
    console.log("Sorry but nvm can not install the Node version below v4.5.0");
    process.exit(1);
  }
  ...
  if (common.dirHasNodeBin(nodeDir)) {
    console.log(ck`<red>Node.js version ${version} is already installed</>`);
    process.exit(1);
  }
