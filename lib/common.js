...
module.exports = {
    ...
    resetNvmPaths() {
        const paths = process.env.PATH.split(";").filter(x => !x.startsWith(process.env.NVM_HOME) && x !== process.env.NVM_LINK);
        process.env.PATH = paths.concat(path.join(process.env.NVM_HOME, "bin")).join(";")
    },

    setNvmUsePath(nodeDir) {
        process.env.PATH = [nodeDir].concat(process.env.PATH.split(";")).filter(x => x).join(";");
    },

    setNvmLinkPath() {
        const link = process.env.NVM_LINK;
        if (link && fs.existsSync(link)) {
            process.env.PATH = process.env.PATH.split(";").concat(link).filter(x => x).join(";")
        }
    },
